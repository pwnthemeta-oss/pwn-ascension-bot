...
def handle_reasoning_callback(bot, update):
    ...
    if data.startswith("verify_reason_answer_"):
        ...
        rest = parts[3]
        chosen, correct = rest.rsplit("_", 1)
        return check_reasoning_answer(bot, update, chosen, correct)


# -----------------------------------------------------------
# SCREEN 1 — Ask the reasoning question
# -----------------------------------------------------------
def ask_reasoning_question(bot, update):
    ...
    text = (
        "🔐 *HUMAN VERIFICATION — REASONING TEST*\n\n"
        f"{question}\n\n"
...
def check_reasoning_answer(bot, update, chosen, correct):
    ...
    if chosen == correct:
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🚀 Begin Onboarding", callback_data="onb_next")],
        ])
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔄 Try Again", callback_data="verify_reason_start")],
        ])
